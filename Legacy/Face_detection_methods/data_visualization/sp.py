...
# def signal_process(roi_mean, fps):
#     # # 1. 读取视频数据
...
#     #why are all the coefficents real?

#     print(spectrum_in_range)


#     top5_indices=np.where(spectrum_in_range == np.sort_complex(spectrum_in_range)[-3:])
...
def signal_process(roi_mean):
    ...
    # print("spectrum_in_range:", spectrum_in_range)
    # 获取前5个最大幅度的索引
    top5_indices = np.argsort(spectrum_in_range)[-6:-3]
...
def signal_process_alter(pix_intensity, fps):
    ...
    average=WaveLet_heart_rate*60

    return average


def signal_process_alter_amplitude(pix_intensity, fps):
    sampling_rate = fps
...
if __name__ == '__main__':
    ...
    print(signal_process(compute_roi_mean(Amplitude_[0])))
