    ...
    for frame_idx in range(0, len(frames), frame_interval):
        ...
        if bbox is not None:
            ...
            for fnum in range(max(0, frame_idx - frame_interval+1), frame_idx+1):
                ...
                rect_width = bbox[2] // X_SIZE
                rect_height = bbox[3] // Y_SIZE

                # List to hold the smaller arrays
                sub_array = np.zeros((X_SIZE, Y_SIZE), dtype=np.ndarray)
                ...
                for i in range(X_SIZE):
                    for j in range(Y_SIZE):
                        ...
                        y_start = j * rect_height
                        y_end = (j + 1) * rect_height
                        
                        # Extract the sub-array corresponding to this grid cell
                        sub_array[j,i] = np.array(face[y_start:y_end, x_start:x_end], dtype=np.float32)
...
if __name__=="__main__":
    ...
    data_output_path='/Users/henryschnieders/Desktop/Research/My_work/Real_time_recognition/All_face_sections/Data/'+f'{vid_name}'+'_frames.npy'
